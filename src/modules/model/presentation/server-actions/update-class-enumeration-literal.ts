...
import { UpdateClassEnumerationLiteralDTO, type UpdateClassEnumerationLiteralDTOProps } from "../dtos/update-class-enumeration-literal";

export const updateClassEnumerationLiteral = withAuth(
  async (payload: UpdateClassEnumerationLiteralDTOProps, { token }): Promise<UpdateClassEnumerationLiteralUseCaseResponse> => {
    const dto = UpdateClassEnumerationLiteralDTO.create(payload);

    const useCase = new UpdateClassEnumerationLiteralUseCase(classDiagramRepository);
