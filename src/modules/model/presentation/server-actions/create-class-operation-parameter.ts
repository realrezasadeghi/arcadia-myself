...
import {
  ...
  type CreateClassOperationParameterDTOProps,
} from "../dtos/create-class-operation-parameter";

export const createClassOperationParameter = withAuth(
  async (
    payload: CreateClassOperationParameterDTOProps,
    { token },
  ): Promise<CreateClassOperationParameterUseCaseResponse> => {
    const dto = CreateClassOperationParameterDTO.create(payload);

    const useCase = new CreateClassOperationParameterUseCase(
