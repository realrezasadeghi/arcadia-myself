...
import { UpdateClassOperationParameterDTO, type UpdateClassOperationParameterDTOProps } from "../dtos/update-class-operation-parameter";

export const updateClassOperationParameter = withAuth(
  async (payload: UpdateClassOperationParameterDTOProps, { token }): Promise<UpdateClassOperationParameterUseCaseResponse> => {
    const dto = UpdateClassOperationParameterDTO.create(payload);

    const useCase = new UpdateClassOperationParameterUseCase(classDiagramRepository);
